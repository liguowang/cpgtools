...
def main():
	...
	parser.add_option("-f","--family",action="store",type="int",dest="family_func",default=2, help="A gamlss (https://cran.r-project.org/web/packages/gamlss/index.html) family object. Can be integer 1 or 2 with 1 = \"BB (beta binomial)\" and 2 = \"ZIBB (zero inflated beta binomial)\". Default=%default.")
	...
	for l in open(options.out_file + '.r.results.txt'):
		...
		if len(tmp)%3 == 0:
			...
			v_names = sub_lists[0][1:-2]
			v_pvals = sub_lists[1][1:-2]
			v_coefs = sub_lists[2][1:-2]
			bbr_results[cgID] = [v_coefs,v_pvals]
		else:
			bbr_results[cgID] = [["NaN"]* len(cv_names), ["NaN"]* len(cv_names)]
	...
	for l in ireader.reader(options.input_file):
		...
		if line_num == 1:
			...
			cgID = f[0]
			print (l + '\t' + '\t'.join(bbr_results[cgID][0]) + '\t' + '\t'.join(bbr_results[cgID][1]), file=FOUT)
